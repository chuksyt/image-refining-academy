...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-18 py-3">
          ...
          <Link href="/" className="flex items-center gap-2 group">
            ...
            <div className="flex flex-col leading-tight">
              <span className={`font-bold text-sm tracking-wide transition-colors ${scrolled ? 'text-gray-900' : 'text-white'}`}>
                Image Refining
              </span>
              <span className={`text-xs transition-colors ${scrolled ? 'text-violet-600' : 'text-purple-300'}`}>
                Academy
              </span>
            </div>
          </Link>
          ...
          >
            <span className={`block w-6 h-0.5 transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''} ${scrolled ? 'bg-gray-800' : 'bg-white'}`} />
            <span className={`block w-6 h-0.5 transition-all duration-300 ${menuOpen ? 'opacity-0' : ''} ${scrolled ? 'bg-gray-800' : 'bg-white'}`} />
            <span className={`block w-6 h-0.5 transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''} ${scrolled ? 'bg-gray-800' : 'bg-white'}`} />
          </button>
        </div>
      </nav>
      ...
      {/* Mobile menu */}
      <div className={`fixed inset-x-0 top-0 z-40 bg-white shadow-xl pt-20 pb-6 px-6 flex flex-col gap-3 transition-all duration-300 md:hidden ${menuOpen ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
        {links.map(l => (
          <Link
            key={l.href}
            href={l.href}
            className={`text-base font-medium py-2 border-b border-gray-100 ${pathname === l.href ? 'text-violet-600' : 'text-gray-700'}`}
          >
            {l.label}
          </Link>
        ))}
