...
const inputCls =
  'w-full rounded-lg border border-gray-300 px-2 py-1.5 text-gray-900 text-sm focus:border-violet-500 focus:ring-2 focus:ring-violet-200 outline-none'
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={addByUrl} className="border border-gray-300 text-gray-700 font-medium px-4 py-2.5 rounded-xl hover:border-violet-400">＋ Add by URL / YouTube</button>
        <div className="flex-1" />
        <button onClick={save} disabled={saving || !dirty} className="bg-gray-900 text-white font-bold px-6 py-2.5 rounded-xl hover:bg-gray-800 transition-all disabled:opacity-40">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((it, idx) => (
          <div key={it.id} className="bg-white border border-gray-200 rounded-2xl p-3 space-y-2">
            <div className="relative h-40 rounded-xl overflow-hidden bg-gray-100 flex items-center justify-center">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              {it.src ? <img src={it.src} alt={it.caption} className="w-full h-full object-cover" /> : <span className="text-gray-400 text-sm">No image</span>}
              <div className="absolute top-2 right-2 flex gap-1">
                <button onClick={() => move(it.id, -1)} disabled={idx === 0} title="Move up" className="w-7 h-7 rounded-full bg-white/90 text-gray-700 disabled:opacity-30 shadow">↑</button>
                <button onClick={() => move(it.id, 1)} disabled={idx === items.length - 1} title="Move down" className="w-7 h-7 rounded-full bg-white/90 text-gray-700 disabled:opacity-30 shadow">↓</button>
              </div>
            </div>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => removeItem(it.id)} className="text-sm text-red-600 hover:text-red-800 px-2 whitespace-nowrap">Remove</button>
            </div>
      ...
      {items.length === 0 && <p className="text-sm text-gray-400 text-center py-10">No images yet. Upload one to get started.</p>}
    </div>
  )
}
