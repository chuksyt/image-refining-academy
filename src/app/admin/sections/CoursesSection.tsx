    ...
    <div className="grid lg:grid-cols-[300px_1fr] gap-8">
      <aside>
        ...
        <div className="space-y-2">
          {courses.map(c => (
            <button key={c.key} onClick={() => openEdit(c)} className={`w-full text-left rounded-xl px-4 py-3 border transition-colors ${originalKey === c.key && !isNew ? 'border-violet-400 bg-violet-50 dark:border-violet-500 dark:bg-violet-950/40' : 'border-gray-200 bg-white hover:border-violet-300 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-violet-700'}`}>
              <div className="flex items-center gap-2">
                <span className="font-medium text-gray-900 dark:text-white text-sm line-clamp-1">{c.name}</span>
                {c.popular && <span className="text-[10px] font-bold text-violet-600 dark:text-violet-300 bg-violet-100 dark:bg-violet-950/50 px-1.5 py-0.5 rounded-full flex-shrink-0">POPULAR</span>}
              </div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{c.priceDisplay} · {c.tag}</div>
            </button>
          ))}
        </div>
      </aside>
      ...
      <section>
        {!draft ? (
          <div className="bg-white dark:bg-gray-900 border border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-16 text-center text-gray-400 dark:text-gray-500">Select a programme to edit, or add a new one.</div>
        ) : (
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-gray-900 dark:text-white">{isNew ? 'New programme' : 'Edit programme'}</h2>
              <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                <input type="checkbox" checked={draft.popular} onChange={e => set('popular', e.target.checked)} className="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-violet-600 focus:ring-violet-500" />
                Mark as popular
              </label>
            </div>

            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Programme name</label><input value={draft.name} onChange={e => onNameChange(e.target.value)} className={inputCls} /></div>

            <div className="grid sm:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price <span className="text-gray-400 dark:text-gray-500 font-normal">(₦, numbers)</span></label>
                <input value={draft.price} onChange={e => onPriceChange(e.target.value)} inputMode="numeric" className={inputCls} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price label</label>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tag <span className="text-gray-400 dark:text-gray-500 font-normal">(e.g. &quot;Ages 6 – 12&quot;)</span></label><input value={draft.tag} onChange={e => set('tag', e.target.value)} className={inputCls} /></div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Key <span className="text-gray-400 dark:text-gray-500 font-normal">(internal id — used by enrollment; avoid changing)</span></label>
              <input value={draft.key} onChange={e => { setKeyTouched(true); set('key', e.target.value) }} className={inputCls} />
            </div>

            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label><textarea value={draft.description} onChange={e => set('description', e.target.value)} rows={3} className={inputCls} /></div>

            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Features <span className="text-gray-400 dark:text-gray-500 font-normal">— one per line</span></label><textarea value={draft.featuresText} onChange={e => set('featuresText', e.target.value)} rows={5} className={inputCls} /></div>

            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Image URL</label><input value={draft.image} onChange={e => set('image', e.target.value)} className={inputCls} /></div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
              <div>
                {!isNew && (confirmingDelete ? (
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600 dark:text-gray-300">Delete this programme?</span>
