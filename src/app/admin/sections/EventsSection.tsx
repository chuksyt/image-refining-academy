    ...
    <div className="grid lg:grid-cols-[300px_1fr] gap-8">
      <aside>
        ...
        <div className="space-y-2">
          {intakes.map(i => (
            <button key={i.id} onClick={() => openEdit(i)} className={`w-full text-left rounded-xl px-4 py-3 border transition-colors ${draft?.id === i.id && !isNew ? 'border-violet-400 bg-violet-50' : 'border-gray-200 bg-white hover:border-violet-300'}`}>
              <span className="font-medium text-gray-900 text-sm line-clamp-1">{i.courseName}</span>
              <div className="text-xs text-gray-400 mt-0.5">{i.date} · {i.spotsLeft}/{i.spotsTotal} spots</div>
            </button>
          ))}
        </div>
      </aside>
      ...
      <section>
        {!draft ? (
          <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-16 text-center text-gray-400">Select an event to edit, or add a new one.</div>
        ) : (
          <div className="bg-white border border-gray-200 rounded-2xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-gray-900">{isNew ? 'New event' : 'Edit event'}</h2>
              <label className="flex items-center gap-2 text-sm text-gray-600">
                <input type="checkbox" checked={draft.rolling} onChange={e => set('rolling', e.target.checked)} className="rounded border-gray-300 text-violet-600 focus:ring-violet-500" />
                Rolling enrollment
              </label>
            </div>

            <div><label className="block text-sm font-medium text-gray-700 mb-1">Programme name</label><input value={draft.courseName} onChange={e => set('courseName', e.target.value)} className={inputCls} /></div>

            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Date <span className="text-gray-400 font-normal">(display)</span></label><input value={draft.date} onChange={e => set('date', e.target.value)} placeholder="July 14, 2026" className={inputCls} /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">ISO date <span className="text-gray-400 font-normal">(YYYY-MM-DD, for sorting)</span></label><input value={draft.isoDate} onChange={e => set('isoDate', e.target.value)} placeholder="2026-07-14" className={inputCls} /></div>
            </div>

            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Format</label><input value={draft.format} onChange={e => set('format', e.target.value)} className={inputCls} /></div>
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Total spots</label><input value={draft.spotsTotal} onChange={e => set('spotsTotal', e.target.value)} inputMode="numeric" className={inputCls} /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Spots left</label><input value={draft.spotsLeft} onChange={e => set('spotsLeft', e.target.value)} inputMode="numeric" className={inputCls} /></div>
              <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Price label</label><input value={draft.price} onChange={e => set('price', e.target.value)} placeholder="₦35,000" className={inputCls} /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Course key <span className="text-gray-400 font-normal">(links to a programme)</span></label><input value={draft.courseKey} onChange={e => set('courseKey', e.target.value)} className={inputCls} /></div>
            </div>
