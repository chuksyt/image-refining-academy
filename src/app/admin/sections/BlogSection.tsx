        ...
        <div className="space-y-2">
          {posts.map(post => (
            <button key={post.slug} onClick={() => openEdit(post)}
              className={`w-full text-left rounded-xl px-4 py-3 border transition-colors ${originalSlug === post.slug && !isNew ? 'border-violet-400 bg-violet-50 dark:border-violet-500 dark:bg-violet-950/40' : 'border-gray-200 bg-white hover:border-violet-300 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-violet-700'}`}>
              <div className="flex items-center gap-2">
                <span className="font-medium text-gray-900 dark:text-white text-sm line-clamp-1">{post.title}</span>
                {post.featured && <span className="text-[10px] font-bold text-violet-600 dark:text-violet-300 bg-violet-100 dark:bg-violet-950/50 px-1.5 py-0.5 rounded-full flex-shrink-0">FEATURED</span>}
              </div>
              <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{post.category} · {post.date}</div>
            </button>
          ))}
          {posts.length === 0 && <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-8">No posts yet.</p>}
        </div>
      </aside>

      <section>
        {!draft ? (
          <div className="bg-white dark:bg-gray-900 border border-dashed border-gray-300 dark:border-gray-700 rounded-2xl p-16 text-center text-gray-400 dark:text-gray-500">
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-gray-900 dark:text-white">{isNew ? 'New post' : 'Edit post'}</h2>
              <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
                <input type="checkbox" checked={draft.featured} onChange={e => set('featured', e.target.checked)} className="rounded border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-violet-600 focus:ring-violet-500" />
                Featured post
              </label>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Slug <span className="text-gray-400 dark:text-gray-500 font-normal">(URL)</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Content <span className="text-gray-400 dark:text-gray-500 font-normal">— separate paragraphs with a blank line</span></label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Cover image URL <span className="text-gray-400 dark:text-gray-500 font-normal">(or a path like /chinenye2.jpg)</span></label>
              <input value={draft.image} onChange={e => set('image', e.target.value)} className={inputCls} />
            </div>

            <div className="grid sm:grid-cols-3 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Author</label><input value={draft.author} onChange={e => set('author', e.target.value)} className={inputCls} /></div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Author role</label><input value={draft.authorRole} onChange={e => set('authorRole', e.target.value)} className={inputCls} /></div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Author avatar URL</label><input value={draft.authorAvatar} onChange={e => set('authorAvatar', e.target.value)} className={inputCls} /></div>
            </div>

            <div className="grid sm:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Date</label><input value={draft.date} onChange={e => set('date', e.target.value)} className={inputCls} /></div>
              <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Read time</label><input value={draft.readTime} onChange={e => set('readTime', e.target.value)} className={inputCls} /></div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
              <div>
                {!isNew && (confirmingDelete ? (
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-600 dark:text-gray-300">Delete this post?</span>
